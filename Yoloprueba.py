...
class YOLOVideoProcessor(VideoTransformerBase):
    ...
    def transform(self, frame):
        img = frame.to_ndarray(format="bgr24")  # convierte a array compatible con OpenCV
...
class Inference:
    ...
    def web_ui(self) -> None:
        ...
        font-family: 'Archivo', sans-serif; margin-bottom:20px;">Prueba del Modelo YOLO11</h1></div>"""
    ...
    def sidebar(self) -> None:
        ...
        with self.st.sidebar:  # Add Ultralytics LOGO
            logo = "https://raw.githubusercontent.com/Mbenitez11/YOLO11webSolarTest/refs/heads/main/assets/logo/fiuna.svg"
        ...
        self.iou = float(self.st.sidebar.slider("Umbral de coincidencia entre caja detectada y objeto real", 0.0, 1.0, self.iou, 0.01))  # Slider for NMS threshold

        if self.source != "imagen":  # Only create columns for video/webcam
    ...
    def source_upload(self) -> None:
        ...
        if self.source == "video":
            vid_file = self.st.sidebar.file_uploader("Subir archivo de video", type=VID_FORMATS)
            ...
            imgfiles = self.st.sidebar.file_uploader("Subir archivo de imagen", type=IMG_FORMATS, accept_multiple_files=True)
            if imgfiles:
    ...
    def configure(self) -> None:
        ...
        selected_model = self.st.sidebar.selectbox("Modelo", available_models)

        with self.st.spinner("El modelo se está cargando..."):
            self.model = YOLO(f"{selected_model.lower()}.pt")  # Load the YOLO model
            class_names = list(self.model.names.values())  # Convert dictionary to list of class names
        self.st.success("Modelo cargado correctamente!")

        # Multiselect box with class names and get indices of selected classes
        selected_classes = self.st.sidebar.multiselect("Clases", class_names, default=class_names[:3])
    ...
    def image_inference(self) -> None:
        ...
        for idx, img_info in enumerate(self.img_file_names):
            ...
            if image is not None:
                ...
                with col1:
                    self.st.image(image, channels="BGR", caption="🖼 Imagen original")
                results = self.model(image, conf=self.conf, iou=self.iou, classes=self.selected_ind)
                annotated_image = results[0].plot()
                with col2:
                    self.st.image(annotated_image, channels="BGR", caption="✅ Imagen procesada")
                try:  # Clean up temporary file
                    os.unlink(img_path)
                except FileNotFoundError:
                    pass  # File doesn't exist, ignore
            else:
                self.st.error("No se pudo cargar la imagen.")
    ...
    def inference(self) -> None:
        ...
        if self.source == "webcam cliente":
           self.st.markdown("### 📸 Usando la cámara del dispositivo cliente")
           webrtc_streamer(
             key="realtime",
        ...
        if self.st.sidebar.button("▶ Iniciar detección"):
            if self.source == "imagen":
                if self.img_file_names:
                    self.image_inference()
                else:
                    self.st.info("Por favor, alza un archivo de imagen para iniciar.")
                return

            stop_button = self.st.sidebar.button("⏹ Detener")  # Button to stop the inference
            cap = cv2.VideoCapture(self.vid_file_name)  # Capture the video
            if not cap.isOpened():
                self.st.error("No se pudo abrir la fuente de video.")
                return

            while cap.isOpened():
                success, frame = cap.read()
                if not success:
                    self.st.warning("Error de video. Por favor verifica la conexion de la webcam.")
                    break
                ...
                self.org_frame.image(frame, channels="BGR", caption="🎥 Frame original")  # Display original frame
                self.ann_frame.image(annotated_frame, channels="BGR", caption="📌 Frame con detección")  # Display processed
...
if __name__ == "__main__":
    Inference(model="yolo11n.pt").inference()
